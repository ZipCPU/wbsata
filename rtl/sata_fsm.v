...
module	sata_fsm #(
		...
		output	reg	[1:0]		o_mm2s_transferlen,
	...
	if (i_reset || !i_wb_stb || !i_wb_we || o_wb_stall)
		cmd_request <= 0;
	else
		cmd_request <= (i_wb_addr == 0)
			&& i_wb_sel[0] && i_wb_data[7:0] == FIS_REG_TO_DEV;
	// }}
	...
	begin
		...
		FSM_PIO_OUT_SETUP: begin
			...
			begin
				...
				o_tran_req <= 1;
				o_tran_src <= SRC_MM2S;
				o_tran_len <= (dma_length >= 2048) ? 2048 : dma_length[11:0]; // DATA_FIS
			end end
			// }}}
		...
		FSM_DMA_OUT_SETUP: begin // DMA write to device
			...
			o_mm2s_request <= 0;
			o_tran_src <= SRC_MM2S;
			o_tran_len <= (dma_length > 2048) ? 2048 : dma_length[11:0];
			if (s_pkt_valid && s_sop
					&& s_data[7:0] == FIS_REG_TO_HOST)
				fsm_state <= FSM_IDLE;
			else if (s_pkt_valid && s_sop
...
`ifdef	FORMAL
	...
	begin
		assert(o_mm2s_request);
		assert($stable(o_mm2s_transferlen));
